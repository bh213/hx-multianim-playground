...
class CheckboxTestScreen extends UIScreenBase {
	...
	public function load():Void {
		...
		var ui = MacroUtils.macroBuildWithParameters(checkboxBuilder, "ui", [], [
			checkbox=>addCheckbox(builder, false),
		]);
	...
	public function onScreenEvent(event:UIScreenEvent, source:UIElement) {
		switch event {
			case UIToggle(pressed):
				this.updatableText.updateText('${pressed}');
			default:
		}
	}
} 